...
def enc_and_process(image, HE, model_encoded):
...
def experiment_encrypted():
    ...
    for batch in experiment_loader:
        image, labels = batch
        preds = enc_and_process(image[0], HE, model_encoded)
        return
...
if __name__ == '__main__':
    ...
    experiment_encrypted()
    total_time = time.time() - starting_time
    log.info(f"The encrypted processing of one image required {total_time}")
